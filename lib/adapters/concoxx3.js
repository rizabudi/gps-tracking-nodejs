...
exports.protocol = 'CONCOXX3';
exports.model_name = 'CONCOXX3';
exports.compatible_hardware = ['CONCOXX3/supplier'];
...
var adapter = function (device) {
  ...
  this.parse_data = function (data) {
    ...
    if (parts['start'] == '7878') {
      ...
      if (parts['protocal_id'] == '01') {
        ...
        parts.cmd = 'action';
        parts.action = 'action';
      } else if (parts['protocal_id'] == '26' || parts['protocal_id'] == '19') {
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'alarm';
        parts.action = 'alarm';
      ...
      if (parts['protocal_id'] == '21') {
        parts['device_id'] = '';
        parts['data'] = data.substr(10, parts['length'] * 2);
        parts.cmd = 'action';
        parts.action = 'action';
      } else if (parts['protocal_id'] == '94') {
        parts['device_id'] = '';
        parts['data'] = data.substr(10, parts['length'] * 2);
        parts.cmd = 'heartbeat';
        parts.action = 'heartbeat';
  ...
  this.receive_heartbeat = function (msg_parts) {
    ...
    var protocal_id = msg_parts.protocal_id;
    ...
    if(protocal_id == '13') {
      data = {
        ...
        // 'accv': this.dex_to_dec(str.substr(2, 4))/100,
        // 'power': this.dex_to_dec(str.substr(6, 2)),
        'gsm': this.dex_to_dec(str.substr(8, 2)),
        'alert': '',
      };
    ...
    } else if(protocal_id == '94') {
      if(str.substr(0,2) == '00') {
        data['accv'] = this.dex_to_dec(str.substr(2,str.length - 12))/100
      }
    }
  ...
  this.receive_action = function (msg_parts) {
    ...
    var server_flag = str.substr(0, 8);

    var data = {
      'length_command'  : str.length,
  ...
  this.get_ping_data = function (msg_parts) {
    ...
    var data = {
      ...
      'acc': parseInt(str.substr(52, 2), 16),
    };
