...
exports.protocol = 'GT710';
exports.model_name = 'GT710';
exports.compatible_hardware = ['GT710/supplier'];
...
var adapter = function (device) {
  ...
  this.parse_data = function (data) {
    ...
    if (parts['start'] == '7878') {
      ...
      if (parts['protocal_id'] == '01') {
        ...
        parts.cmd = 'ping';
        parts.action = 'ping';
      } else if (parts['protocal_id'] == '23' || parts['protocal_id'] == '13') {
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'heartbeat';
        parts.action = 'heartbeat';
      } else if (parts['protocal_id'] == '21') {
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'action';
        parts.action = 'action';
      } else if (parts['protocal_id'] == '26' || parts['protocal_id'] == '18') {
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'alarm';
        parts.action = 'alarm';
  ...
  this.receive_alarm = function (msg_parts) {
    ...
    var dateOjb = new Date(2000 + year, month-1, day, hour+7, min, sec);
    data['date'] = dateOjb.toISOString().replace(/T/, ' ').replace(/\..+/, '');
  ...
  this.get_ping_data = function (msg_parts) {
    ...
    var dateOjb = new Date(2000 + year, month-1, day, hour, min, sec);
    data['date'] = dateOjb.toISOString().replace(/T/, ' ').replace(/\..+/, '');
