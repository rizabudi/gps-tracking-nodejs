...
function Device(adapter, connection, gpsServer) {
  ...
  /****************************************
   RECEIVING ACTION
   ****************************************/
  this.receive_action = function (msgParts) {
    //We pass the message parts to the adapter and they have to say wich type of alarm it is.
    var actionData = _this.adapter.receive_action(msgParts);
    /* Alarm data must return an object with at least:
     alarm_type: object with this format:
     {'code':'sos_alarm','msg':'SOS Alarm activated by the driver'}
     */
    _this.emit('action', actionData, msgParts);
  };
  ...
  this.receive_heartbeat = function (msgParts) {
    //We pass the message parts to the adapter and they have to say wich type of alarm it is.
    var actionData = _this.adapter.receive_heartbeat(msgParts);
    /* Alarm data must return an object with at least:
     alarm_type: object with this format:
     {'code':'sos_alarm','msg':'SOS Alarm activated by the driver'}
     */
    _this.emit('heartbeat', actionData, msgParts);
  };
  ...
  this.send_command = function(id, cmd, type) {
    var msg = this.adapter.get_command(id, cmd, type)
    this.send(msg);
  }

  this.send_response = function(action) {
    var msg = this.adapter.get_response(action)
    if(msg !== undefined) {
      this.send(msg);
    }
  }
